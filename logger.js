...
const logInteraction = async(inter) => { //inp interaction
	...
	fs.appendFileSync(`${path}${fileName}`,inp)
	console.log(`${chalk.gray(`[${d}]`)} ${chalk.blue(inter.guild.name)} ${chalk.green(inter.channel.name)} ${chalk.red(inter.member?.displayName)}\n${chalk.black(chalk.bold(inter.commandName))}`)
}
...
export {logMessage,logEdit,logInteraction,logMember,logGuild}
